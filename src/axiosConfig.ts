...
const obtenerNuevoToken = async () => {
  ...
  if(refresh_token === null) throw new Error("no existe refresh token, inicia sesion")
  ...
  const {data} = await axios.post("https://accounts.spotify.com/api/token", refreshTokenParams,{
    headers:{
...
axios.interceptors.response.use(
  ...
  async error => {
    ...
    if (error.response.status === 401) { // Comprobamos si el error es por token expirado
      try {
