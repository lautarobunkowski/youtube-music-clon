...
const Player = () => {
  ...
  useEffect(() => {
    const fetchData = async() => {
      const {data} = await axios(`https://api.spotify.com/v1/tracks/${currentSong.id}`)
      console.log(data)
      setTrack(data)
    }
    if (isPlaying) {
      ...
      <div className="w-full mx-4 flex justify-between items-center">
        ...
        <div className="flex gap-6 items-center text-zinc-500">
          <div className="flex items-center">
            <div className={`w-20 transition-all duration-200 ${showSoundSlider?"opacity-100 visible cursor-pointer":"opacity-0 invisible"}`}>
              <Slider className="bg-zinc-500 flex "/>
            </div>
            <div onMouseEnter={() => setShowSoundSlider(true)} className="pl-4 cursor-pointer">
              <Volume/>
            </div>
          </div>
